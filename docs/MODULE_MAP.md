...
| `project-loop.js` | `loop_start`, `loop_stop`, `ralph_wizard_complete`, `ralph_wizard_cancel`, `ralph_cancel_crafting`, `ralph_preview_files`, `loop_registry_*`, `schedule_create`, `hub_schedules_list`, `delete_loop_group`, `loop_library_scan`, `loop_library_read`, `loop_library_load`, `loop_library_run` | Loop/Ralph engine, loop registry, scheduling, loop library |
