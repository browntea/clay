...
function attachLoop(ctx) {
  ...
  // --- Loop Library ---
  var libraryBase = path.join(cwd, ".claude", "loop-library");

  function scanLibrary() {
    var entries = [];
    try {
      var dirs = fs.readdirSync(libraryBase);
      for (var i = 0; i < dirs.length; i++) {
        var slug = dirs[i];
        var slugDir = path.join(libraryBase, slug);
        try {
          var stat = fs.statSync(slugDir);
          if (!stat.isDirectory()) continue;
        } catch (e) { continue; }
        // PROMPT.md is required
        var promptPath = path.join(slugDir, "PROMPT.md");
        try { fs.accessSync(promptPath); } catch (e) { continue; }
        var hasJudge = false;
        try { fs.accessSync(path.join(slugDir, "JUDGE.md")); hasJudge = true; } catch (e) {}
        var meta = {};
        try { meta = JSON.parse(fs.readFileSync(path.join(slugDir, "LOOP.json"), "utf8")); } catch (e) {}
        entries.push({
          slug: slug,
          description: meta.description || "",
          tags: meta.tags || [],
          maxIterations: meta.maxIterations || 20,
          hasJudge: hasJudge,
          version: meta.version || 1,
        });
      }
    } catch (e) {
      // loop-library directory doesn't exist yet — that's fine
    }
    return entries;
  }

  function readLibraryEntry(slug) {
    var slugDir = path.join(libraryBase, slug);
    var prompt = "";
    var judge = "";
    var meta = {};
    try { prompt = fs.readFileSync(path.join(slugDir, "PROMPT.md"), "utf8"); } catch (e) {}
    try { judge = fs.readFileSync(path.join(slugDir, "JUDGE.md"), "utf8"); } catch (e) {}
    try { meta = JSON.parse(fs.readFileSync(path.join(slugDir, "LOOP.json"), "utf8")); } catch (e) {}
    return { slug: slug, prompt: prompt, judge: judge, meta: meta };
  }

  function loadLibraryEntry(slug) {
    if (loopState.active || loopState.phase === "executing") {
      return { ok: false, error: "A loop is already running" };
    }
    var entry = readLibraryEntry(slug);
    if (!entry.prompt) {
      return { ok: false, error: "PROMPT.md missing for " + slug };
    }
    // Copy to loops/ (standard execution flow)
    var newLoopId = generateLoopId();
    var destDir = path.join(cwd, ".claude", "loops", newLoopId);
    try { fs.mkdirSync(destDir, { recursive: true }); } catch (e) {}
    // Write PROMPT.md
    var tmpP = path.join(destDir, "PROMPT.md.tmp");
    fs.writeFileSync(tmpP, entry.prompt);
    fs.renameSync(tmpP, path.join(destDir, "PROMPT.md"));
    // Write JUDGE.md (if present)
    if (entry.judge) {
      var tmpJ = path.join(destDir, "JUDGE.md.tmp");
      fs.writeFileSync(tmpJ, entry.judge);
      fs.renameSync(tmpJ, path.join(destDir, "JUDGE.md"));
    }
    // Write LOOP.json
    var loopJson = Object.assign({}, entry.meta);
    var tmpL = path.join(destDir, "LOOP.json.tmp");
    fs.writeFileSync(tmpL, JSON.stringify(loopJson, null, 2));
    fs.renameSync(tmpL, path.join(destDir, "LOOP.json"));

    // Register in loop registry
    var maxIter = entry.judge ? (loopJson.maxIterations || 20) : 1;
    loopRegistry.register({
      id: newLoopId,
      name: loopJson.description || slug,
      task: entry.prompt.substring(0, 200),
      source: "library",
      librarySlug: slug,
      maxIterations: maxIter,
    });

    // Set loop state to approval
    ...
    return { ok: true, loopId: newLoopId, maxIterations: maxIter };
  }

  // Library directory watcher
  var libraryWatcher = null;
  var libraryDebounce = null;

  function startLibraryWatch() {
    if (libraryWatcher) return;
    try { fs.mkdirSync(libraryBase, { recursive: true }); } catch (e) {}
    try {
      libraryWatcher = fs.watch(libraryBase, function () {
        if (libraryDebounce) clearTimeout(libraryDebounce);
        libraryDebounce = setTimeout(function () {
          var entries = scanLibrary();
          send({ type: "loop_library_entries", entries: entries });
        }, 300);
      });
      libraryWatcher.on("error", function () {});
    } catch (e) {
      console.error("[ralph-loop] Failed to watch loop-library/:", e.message);
    }
  }
  ...
  // Start library watcher on attach
  startLibraryWatch();
  ...
  function handleLoopMessage(ws, msg) {
    ...
    // --- Loop Library messages ---
    if (msg.type === "loop_library_scan") {
      var entries = scanLibrary();
      sendTo(ws, { type: "loop_library_entries", entries: entries });
      return true;
    }

    if (msg.type === "loop_library_read") {
      var readSlug = msg.slug;
      if (!readSlug) {
        sendTo(ws, { type: "loop_library_error", text: "Missing slug" });
        return true;
      }
      var entry = readLibraryEntry(readSlug);
      if (!entry.prompt) {
        sendTo(ws, { type: "loop_library_error", text: "PROMPT.md missing for " + readSlug });
        return true;
      }
      sendTo(ws, {
        type: "loop_library_entry_content",
        slug: readSlug,
        prompt: entry.prompt,
        judge: entry.judge,
        meta: entry.meta,
    ...
    if (msg.type === "loop_library_load") {
      var loadSlug = msg.slug;
      if (!loadSlug) {
        sendTo(ws, { type: "loop_library_error", text: "Missing slug" });
        return true;
      }
      var loadEntry = readLibraryEntry(loadSlug);
      var hasLoadJudge = !!loadEntry.judge;
      var loadResult = loadLibraryEntry(loadSlug);
      if (!loadResult.ok) {
        sendTo(ws, { type: "loop_library_error", text: loadResult.error });
        return true;
      }
      send({ type: "loop_library_loaded", slug: loadSlug, loopId: loadResult.loopId });
      send({
        type: "ralph_phase",
        phase: "approval",
        source: "library",
      ...
      send({
        type: "ralph_files_status",
        promptReady: true,
        judgeReady: hasLoadJudge,
        bothReady: hasLoadJudge,
        taskId: loadResult.loopId,
      });
      return true;
    }

    if (msg.type === "loop_library_run") {
      var runSlug = msg.slug;
      if (!runSlug) {
        sendTo(ws, { type: "loop_library_error", text: "Missing slug" });
        return true;
      }
      var runResult = loadLibraryEntry(runSlug);
      if (!runResult.ok) {
        sendTo(ws, { type: "loop_library_error", text: runResult.error });
        return true;
      }
      send({ type: "loop_library_loaded", slug: runSlug, loopId: runResult.loopId });
      startLoop({ maxIterations: runResult.maxIterations, name: loopState.wizardData ? loopState.wizardData.name : runSlug });
      return true;
    }
  ...
  function sendConnectionState(ws) {
    ...
    var _libraryCount = 0;
    try {
      var _libDirs = fs.readdirSync(libraryBase);
      for (var _lci = 0; _lci < _libDirs.length; _lci++) {
        var _lcDir = path.join(libraryBase, _libDirs[_lci]);
        try {
          if (!fs.statSync(_lcDir).isDirectory()) continue;
          fs.accessSync(path.join(_lcDir, "PROMPT.md"));
          _libraryCount++;
        } catch (e) {}
      }
    } catch (e) {}
  ...
  return {
    ...
    stopLibraryWatch: stopLibraryWatch,
