...
function attachSessions(ctx) {
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "rewind_preview") {
      ...
      (async function () {
        // Codex: chat-only rewind (no file checkpointing), return empty preview
        if (session.vendor === "codex") {
          sendTo(ws, { type: "rewind_preview_result", preview: { filesChanged: [] }, diffs: {}, uuid: msg.uuid, chatOnly: true });
          return;
        }
        var result;
        try {
    ...
    if (msg.type === "rewind_execute") {
      ...
      // Codex only supports chat rewind (no file checkpointing)
      var mode = session.vendor === "codex" ? "chat" : (msg.mode || "both");

      (async function () {
        var result;
        try {
          // File restoration (skip for chat-only mode and Codex)
          if (mode !== "chat") {
            result = await sdk.getOrCreateRewindQuery(session);
          ...
          if (mode !== "files") {
            ...
            // Codex: count turns to roll back BEFORE trimming local history
            var codexRollbackTurns = 0;
            if (session.vendor === "codex" && targetIdx >= 0 && adapter.rollbackThread) {
              for (var ri = targetIdx; ri < session.messageUUIDs.length; ri++) {
                if (session.messageUUIDs[ri].type === "user") codexRollbackTurns++;
              }
            }
            ...
            // Codex: roll back turns on the app-server thread
            if (codexRollbackTurns > 0) {
              try {
                await adapter.rollbackThread(session.cliSessionId, codexRollbackTurns);
              } catch (rbErr) {
                console.error("[project-sessions] Codex thread/rollback failed:", rbErr.message || rbErr);
              }
            }
    ...
    if (msg.type === "fork_session" && msg.uuid) {
      ...
      if (session.vendor === "codex") {
        // Codex: fork thread via app-server, copy local history up to UUID
        adapter.forkSession(forkCliId, {}).then(function(result) {
          if (!result || !result.sessionId) throw new Error("Fork returned no session id");
          // Find target UUID index and copy history up to that point
          var targetIdx = -1;
          ...
          var forked = sm.createSession({ vendor: "codex", ownerId: session.ownerId || null }, ws);
          forked.cliSessionId = result.sessionId;
          forked.title = forkTitle;
          forked.history = forkHistory;
          // Rebuild messageUUIDs from copied history
          forked.messageUUIDs = [];
        ...
        // Claude: fork via SDK, read CLI session history
