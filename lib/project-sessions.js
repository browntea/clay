...
function attachSessions(ctx) {
  ...
  // Multi-user mode reads per-user storage; single-user mode uses "_default"
  // sentinel which maps to the same users-preferences auto-create logic.
  function getClaudeOpenModeForWs(ws) {
    if (!usersModule || typeof usersModule.getClaudeOpenMode !== "function") return "gui";
    var uid = ws && ws._clayUser ? ws._clayUser.id : "_default";
  ...
  function handleSessionsMessage(ws, msg) {
    ...
    if (msg.type === "get_daemon_config" || msg.type === "set_pin" || msg.type === "set_keep_awake" ||
        ...
        msg.type === "setup_totp" || msg.type === "show_totp_qr" || msg.type === "disable_totp" || msg.type === "verify_totp" || msg.type === "set_auth_mode") {
      if (usersModule.isMultiUser()) {
    ...
    if (msg.type === "setup_totp") {
      console.log("[sessions] setup_totp received, opts.onSetupTotp:", typeof opts.onSetupTotp);
      if (typeof opts.onSetupTotp === "function") {
        var totpResult = opts.onSetupTotp();
        console.log("[sessions] setup_totp result:", JSON.stringify(totpResult));
        sendTo(ws, { type: "setup_totp_result", ok: totpResult.ok, secret: totpResult.secret, otpauthUrl: totpResult.otpauthUrl, authMode: totpResult.authMode, error: totpResult.error });
      }
      return true;
    }

    if (msg.type === "show_totp_qr") {
      if (typeof opts.onShowTotpQr === "function") {
        var qrResult = opts.onShowTotpQr();
        sendTo(ws, { type: "show_totp_qr_result", ok: qrResult.ok, secret: qrResult.secret, otpauthUrl: qrResult.otpauthUrl, error: qrResult.error });
      }
      return true;
    }

    if (msg.type === "disable_totp") {
      if (typeof opts.onDisableTotp === "function") {
        var dtResult = opts.onDisableTotp();
        sendTo(ws, { type: "disable_totp_result", ok: dtResult.ok, authMode: dtResult.authMode, error: dtResult.error });
      }
      return true;
    }

    if (msg.type === "verify_totp") {
      if (typeof opts.onVerifyTotp === "function") {
        var vtResult = opts.onVerifyTotp(msg.code);
        sendTo(ws, { type: "verify_totp_result", ok: vtResult.ok, authMode: vtResult.authMode, error: vtResult.error });
      }
      return true;
    }

    if (msg.type === "set_auth_mode") {
      if (typeof opts.onSetAuthMode === "function") {
        var amResult = opts.onSetAuthMode(msg.mode);
        sendTo(ws, { type: "set_auth_mode_result", ok: amResult.ok, authMode: amResult.authMode, error: amResult.error });
      }
      return true;
    }
    ...
    if (msg.type === "set_claude_open_mode") {
      ...
      var comUserId = ws._clayUser ? ws._clayUser.id : "_default";
