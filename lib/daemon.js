...
var relay = createServer({
  ...
  onGetDaemonConfig: function () {
    return {
      ...
      totpEnabled: require("./server-auth").isTotpAvailable(),
      authMode: require("./server-auth").getAuthMode(),
  ...
  onSetupTotp: function () {
    var serverAuth = require("./server-auth");
    // Generate new TOTP secret using server-auth's proper base32 encoding
    var secret = serverAuth.generateTotpSecret();
    serverAuth.saveTotpSecret(secret);
    // Reload into server-auth
    serverAuth.loadTotpSecret();
    // Auto-set authMode to "either" if currently "pin"
    if (serverAuth.getAuthMode() === "pin") {
      serverAuth.saveAuthMode("either");
    }
    var _isLoopback = config.host === "0.0.0.0" || config.host === "127.0.0.1" || config.host === "::1" || config.host === "::";
    var hostname = config.host && !_isLoopback ? config.host : require("os").hostname();
    var otpauthUrl = "otpauth://totp/Clay:" + encodeURIComponent(hostname) + "?secret=" + secret + "&issuer=Clay&algorithm=SHA1&digits=6&period=30";
    console.log("[daemon] TOTP setup (web), authMode:", serverAuth.getAuthMode());
    return { ok: true, secret: secret, otpauthUrl: otpauthUrl, authMode: serverAuth.getAuthMode() };
  },
  onShowTotpQr: function () {
    var serverAuth = require("./server-auth");
    var secret = serverAuth.getTotpSecret();
    if (!secret) {
  ...
  onDisableTotp: function () {
    var serverAuth = require("./server-auth");
    var cfgMod = require("./config");
    var totpFile = path.join(cfgMod.CONFIG_DIR, cfgMod.isDevMode ? "totp-secret-dev.json" : "totp-secret.json");
    try {
      if (fs.existsSync(totpFile)) fs.unlinkSync(totpFile);
    } catch (e) {}
    // Reload (will clear the secret)
    serverAuth.loadTotpSecret();
    // Revert authMode to "pin" if it was TOTP-dependent
    var mode = serverAuth.getAuthMode();
    if (mode === "totp" || mode === "either" || mode === "both") {
      serverAuth.saveAuthMode("pin");
    }
    console.log("[daemon] TOTP disabled (web), authMode:", serverAuth.getAuthMode());
    return { ok: true, authMode: serverAuth.getAuthMode() };
  },
  onVerifyTotp: function (code) {
    var serverAuth = require("./server-auth");
    if (!code || !/^\d{6}$/.test(code)) {
      return { ok: false, error: "Invalid code format" };
    }
    if (!serverAuth.isTotpAvailable()) {
      return { ok: false, error: "TOTP not configured" };
    }
    // Use verifyTotpCode from server-auth to check against loaded secret
    var valid = serverAuth.verifyTotpCode(code);
    if (valid) {
      return { ok: true, authMode: serverAuth.getAuthMode() };
    }
    return { ok: false, error: "Invalid code. Try again." };
  },
  onSetAuthMode: function (mode) {
    var serverAuth = require("./server-auth");
    var validModes = ["pin", "totp", "either", "both"];
    if (validModes.indexOf(mode) === -1) {
      return { ok: false, error: "Invalid mode. Must be: " + validModes.join(", ") };
    }
    if ((mode === "totp" || mode === "both" || mode === "either") && !serverAuth.isTotpAvailable()) {
      return { ok: false, error: "TOTP not configured. Setup TOTP first." };
    }
    serverAuth.saveAuthMode(mode);
    console.log("[daemon] Auth mode set to:", mode, "(web)");
    return { ok: true, authMode: mode };
  },
...
var ipc = createIPCServer(socketPath(), function (msg) {
  ...
  switch (msg.cmd) {
    ...
    case "setup_totp": {
      var serverAuth = require("./server-auth");
      var totpSecret = serverAuth.generateTotpSecret();
      serverAuth.saveTotpSecret(totpSecret);
      // Reload TOTP secret into running server
      serverAuth.loadTotpSecret();
      // Auto-set authMode to "either" if it was "pin"
      if (serverAuth.getAuthMode() === "pin") {
        serverAuth.saveAuthMode("either");
      }
      var totpIssuer = "Clay";
      var _isLoopback = config.host === "0.0.0.0" || config.host === "127.0.0.1" || config.host === "::1" || config.host === "::";
      var totpAccount = config.host && !_isLoopback ? config.host : require("os").hostname();
      var otpauthUrl = "otpauth://totp/" + encodeURIComponent(totpIssuer) + ":" + encodeURIComponent(totpAccount) + "?secret=" + totpSecret + "&issuer=" + encodeURIComponent(totpIssuer) + "&digits=6&period=30&algorithm=SHA1";
      return { ok: true, secret: totpSecret, otpauthUrl: otpauthUrl, authMode: serverAuth.getAuthMode() };
    }

    case "get_totp_status": {
      var serverAuth2 = require("./server-auth");
      var _cfgMod = require("./config");
      var totpSecretFile = path.join(_cfgMod.CONFIG_DIR, _cfgMod.isDevMode ? "totp-secret-dev.json" : "totp-secret.json");
      var hasTotpSecret = false;
      try { hasTotpSecret = fs.existsSync(totpSecretFile); } catch (e) {}
      return { ok: true, enabled: hasTotpSecret };
    }

    case "disable_totp": {
      var _cfgMod2 = require("./config");
      var totpSecretFile2 = path.join(_cfgMod2.CONFIG_DIR, _cfgMod2.isDevMode ? "totp-secret-dev.json" : "totp-secret.json");
      try { fs.unlinkSync(totpSecretFile2); } catch (e) {}
      // Reload to clear _totpSecret
      var serverAuth3 = require("./server-auth");
      serverAuth3.loadTotpSecret();
      // Auto-revert authMode to "pin" if it was totp-dependent
      var currentMode = serverAuth3.getAuthMode();
      if (currentMode === "totp" || currentMode === "either" || currentMode === "both") {
        serverAuth3.saveAuthMode("pin");
      }
      return { ok: true, authMode: serverAuth3.getAuthMode() };
    }

    case "set_auth_mode": {
      var serverAuth4 = require("./server-auth");
      var newMode = msg.mode;
      var validModes = ["pin", "totp", "either", "both"];
      if (validModes.indexOf(newMode) === -1) {
        return { ok: false, error: "Invalid mode. Must be: " + validModes.join(", ") };
      }
      // Require TOTP to be set up for totp-dependent modes
      if ((newMode === "totp" || newMode === "both" || newMode === "either") && !serverAuth4.isTotpAvailable()) {
        return { ok: false, error: "TOTP not configured. Run setup_totp first." };
      }
      serverAuth4.saveAuthMode(newMode);
      console.log("[daemon] Auth mode set to:", newMode);
      return { ok: true, authMode: newMode };
    }
