...
function createClaudeAdapter(opts) {
  ...
  var adapter = {
    ...
    createQuery: async function(queryOpts) {
      ...
      // Critical for MiniMax where getContextUsage is unsupported and the SDK
      // cannot measure usage — without this, compaction fails above ~95% context.
      if (queryOpts.autoCompactWindow) {
        sdkOptions.compactionControl = {
          enabled: true,
          contextTokenThreshold: queryOpts.autoCompactWindow,
        };
      }
  ...
  async function createWorkerQuery(queryOpts, claudeOpts, linuxUser) {
    ...
    // Auto-compaction: tell SDK when to trigger context compaction (in tokens).
    // Critical for MiniMax where getContextUsage is unsupported.
    if (queryOpts.autoCompactWindow) {
      queryOptions.compactionControl = {
        enabled: true,
        contextTokenThreshold: queryOpts.autoCompactWindow,
      };
    }
