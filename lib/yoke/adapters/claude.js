...
function createClaudeAdapter(opts) {
  ...
  var adapter = {
    ...
    createQuery: async function(queryOpts) {
      ...
      // Auto-compaction: tell SDK when to trigger context compaction (in tokens).
      // The SDK reads the top-level `autoCompactWindow` option (min 100_000,
      // max 1_000_000 per its zod schema) — NOT a nested compactionControl
      // object. Critical for MiniMax where the upstream 2013 error fires once
      // the request exceeds the real context window; without this the SDK never
      // auto-compacts and the user hits a hard failure.
      if (queryOpts.autoCompactWindow) {
        sdkOptions.autoCompactWindow = queryOpts.autoCompactWindow;
      }
  ...
  async function createWorkerQuery(queryOpts, claudeOpts, linuxUser) {
    ...
    // Auto-compaction: tell SDK when to trigger context compaction (in tokens).
    // The SDK reads top-level `autoCompactWindow` (not a nested
    // compactionControl). Critical for MiniMax 2013 prevention.
    if (queryOpts.autoCompactWindow) {
      queryOptions.autoCompactWindow = queryOpts.autoCompactWindow;
    }
