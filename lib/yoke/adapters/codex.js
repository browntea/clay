...
function createCodexQueryHandle(appServer, queryOpts) {
  ...
  function handleServerEvent(msg) {
    ...
    if (method === "item/tool/requestUserInput" || method === "mcpServer/elicitation/request") {
      ...
      var mcpToolName = mcpMeta.tool || mcpParams.serverName || "mcp_tool";
      console.log("[yoke/codex] MCP approval request:", mcpToolName, "server=" + (mcpParams.serverName || "?"));
      if (canUseTool) {
        canUseTool("mcp__" + (mcpParams.serverName || "unknown") + "__" + (mcpMeta.tool || "call"), mcpParams, {}).then(function(decision) {
          if (decision === true || decision === "session") {
