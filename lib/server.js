        ...
        projects.forEach(function (ctx, s) {
          ...
          for (var i = 0; i < recs.length; i++) {
            allSchedules.push({
              ...
              name: recs[i].name,
              task: recs[i].task || "",
              nextRunAt: recs[i].nextRunAt || null,
              enabled: recs[i].enabled,
              cron: recs[i].cron || null,
              date: recs[i].date || null,
              time: recs[i].time || null,
              allDay: recs[i].allDay || false,
              color: recs[i].color || "",
              source: recs[i].source || null,
              lastRunAt: recs[i].lastRunAt || null,
              projectSlug: s,
