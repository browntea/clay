...
function createServer(opts) {
  ...
  var appHandler = function (req, res) {
    ...
    // MCP Native Host installer (no auth required)
    if (req.method === "GET" && fullUrl === "/install-mcp") {
      serveMcpInstaller(req, res);
      return;
    }
    if (req.method === "GET" && fullUrl === "/native-host/clay-mcp-host.js") {
      serveMcpHostScript(res);
      return;
    }
...
// --- MCP Native Host Installer ---

var _nativeHostDir = path.join(__dirname, "..", "native-host");
var _lastExtensionId = null;

function setExtensionId(id) { if (id) _lastExtensionId = id; }

function serveMcpInstaller(req, res) {
  var host = req.headers.host || "localhost";
  var proto = req.socket.encrypted ? "https" : "http";
  var baseUrl = proto + "://" + host;

  var extId = _lastExtensionId || "__EXTENSION_ID__";

  var script = [
    '#!/bin/bash',
    'set -e',
    '',
    'INSTALL_DIR="$HOME/.clay/mcp-bridge"',
    'mkdir -p "$INSTALL_DIR"',
    '',
    'echo "Downloading Clay MCP Bridge..."',
    'curl -fsSL "' + baseUrl + '/native-host/clay-mcp-host.js" -o "$INSTALL_DIR/clay-mcp-host.js"',
    'chmod +x "$INSTALL_DIR/clay-mcp-host.js"',
    '',
    'EXT_ID="' + extId + '"',
    '',
    ...
    '',
    'echo ""',
    'echo "Clay MCP Bridge installed."',
    'echo "  Host: $INSTALL_DIR/clay-mcp-host.js"',
    'echo ""',
    'echo "Restart your browser for the native host to take effect."',
  ].join("\n");
...
function serveMcpHostScript(res) {
  var filePath = path.join(_nativeHostDir, "clay-mcp-host.js");
...
module.exports = { createServer: createServer, generateAuthToken: generateAuthToken, verifyPin: verifyPin, setExtensionId: setExtensionId };
