function pinPageHtml(opts) {
  var authMode = (opts && opts.authMode) || "pin";
  var pinEnabled = opts && opts.pinEnabled !== false;
  // Determine which UI elements to show based on authMode and pinEnabled
  var showToggle = authMode === "either" && pinEnabled; // tab switcher only when PIN is set
  var showPassField = authMode !== "totp" && pinEnabled; // passphrase input only when PIN is set
  var showTotpField = authMode !== "pin";               // TOTP input always except pure PIN mode
  var bothMode = authMode === "both";                    // show both side by side

  var subtitle = "Enter your passphrase to continue";
  if (authMode === "totp" || (authMode === "either" && !pinEnabled)) subtitle = "Enter your authenticator code";
  if (authMode === "both") subtitle = "Enter passphrase and authenticator code";
  if (authMode === "either" && pinEnabled) subtitle = "Choose your authentication method";

  // Toggle HTML (only for "either" mode with PIN set)
  var toggleHtml = showToggle ?
    '<div class="auth-toggle">' +
    '<button class="auth-toggle-btn active" id="mode-passphrase" type="button">Passphrase</button>' +
    '<button class="auth-toggle-btn" id="mode-totp" type="button">Authenticator</button>' +
    '</div>' : '';

  // Passphrase field
  var passFieldHtml = showPassField ?
    '<div id="pass-field" class="field"><input id="pin" type="password" placeholder="Enter passphrase" autocomplete="current-password"' + (authMode !== "totp" ? ' autofocus' : '') + '></div>' : '';

  // TOTP field (visible by default in totp/both modes, hidden in either mode)
  var totpHidden = (authMode === "either" && pinEnabled) ? ' style="display:none"' : '';
  var totpFieldHtml = showTotpField ?
    '<div id="totp-field" class="field"' + totpHidden + '>' +
    '<label class="field-label" style="font-size:11px;color:var(--text-dimmer);margin-bottom:4px;display:' + (bothMode ? 'block' : 'none') + '">Authenticator code</label>' +
    '<input id="totp" type="tel" maxlength="6" inputmode="numeric" placeholder="6-digit code" autocomplete="one-time-code"' + (authMode === "totp" || (authMode === "either" && !pinEnabled) ? ' autofocus' : '') + '></div>' : '';
  ...
  return '<!DOCTYPE html><html lang="en"><head>' +
    ...
    '.auth-toggle{display:flex;gap:4px;margin:16px 0 12px;background:var(--input-bg);border-radius:10px;padding:3px}' +
    '.auth-toggle-btn{flex:1;padding:8px;border:none;border-radius:8px;background:transparent;color:var(--text-dimmer);font-size:12px;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.15s}' +
    ...
    passFieldHtml +
    totpFieldHtml +
    '<button class="btn" id="submit-btn">' + btnLabel + '</button>' +
    ...
    'var pinEnabled=' + (authMode === "either" && pinEnabled ? 'true' : 'false') + ';' +
    'var currentMode=authMode==="either"?(pinEnabled?"passphrase":"totp"):(authMode==="totp"?"totp":"passphrase");' +
    // Toggle handlers (only for "either" mode with PIN set)
    'if(authMode==="either"&&pinEnabled){' +
    'var modePass=document.getElementById("mode-passphrase");' +
    'var modeTotp=document.getElementById("mode-totp");' +
    'var passField=document.getElementById("pass-field");' +
    'var totpField=document.getElementById("totp-field");' +
    'modePass.addEventListener("click",function(){' +
    'currentMode="passphrase";' +
    'modePass.classList.add("active");modeTotp.classList.remove("active");' +
    'passField.style.display="";totpField.style.display="none";' +
    'pinInput.focus()});' +
    'modeTotp.addEventListener("click",function(){' +
    'currentMode="totp";' +
    'modeTotp.classList.add("active");modePass.classList.remove("active");' +
    'passField.style.display="none";totpField.style.display="";' +
    'totpInput.focus()});' +
    '}' +
    // Submit function
    'function submitPin(){' +
    'var payload={};' +
    'if(authMode==="both"){' +
    // Both mode: send both fields
    'payload.pin=pinInput?pinInput.value:"";' +
    'payload.totp=totpInput?totpInput.value:"";' +
    'if(!payload.pin||!payload.totp||payload.totp.length!==6)return' +
    '}else if(authMode==="totp"||(authMode==="either"&&currentMode==="totp")){' +
    'payload.totp=totpInput.value;if(!payload.totp||payload.totp.length!==6)return' +
    '}else{' +
    'payload.pin=pinInput.value;if(!payload.pin)return}' +
    ...
    'if(d.locked){' +
    'if(pinInput)pinInput.disabled=true;if(totpInput)totpInput.disabled=true;submitBtn.disabled=true;' +
    'err.textContent="Too many attempts. Try again in "+Math.ceil(d.retryAfter/60)+" min";' +
    'setTimeout(function(){if(pinInput)pinInput.disabled=false;if(totpInput)totpInput.disabled=false;submitBtn.disabled=false;' +
    'if(pinInput)pinInput.value="";if(totpInput)totpInput.value="";err.textContent=""},d.retryAfter*1000);return}' +
    'var msg="Incorrect credentials";' +
    'if(authMode==="either")msg="Incorrect "+(currentMode==="passphrase"?"passphrase":"code");' +
    'if(typeof d.attemptsLeft==="number"&&d.attemptsLeft<=3)msg+=" ("+d.attemptsLeft+" left)";' +
    'err.textContent=msg;' +
    'if(pinInput&&(currentMode==="passphrase"||authMode==="both")){pinInput.value="";pinInput.focus()}' +
    'else if(totpInput){totpInput.value="";totpInput.focus()}})' +
    '.catch(function(){err.textContent="Connection error";submitBtn.disabled=false})}' +
    // Key handlers
    'if(pinInput)pinInput.addEventListener("keydown",function(e){if(e.key==="Enter")submitPin()});' +
    'if(totpInput){' +
    'totpInput.addEventListener("keydown",function(e){if(e.key==="Enter")submitPin()});' +
    'totpInput.addEventListener("input",function(){if(totpInput.value.length===6){' +
    'if(authMode!=="both")submitPin()' +
    '}});}' +
