function pinPageHtml(opts) {
  ...
  // Determine which UI elements to show based on authMode
  var showToggle = authMode === "either";         // tab switcher
  var showPassField = authMode !== "totp";         // passphrase input
  var showTotpField = authMode !== "pin";          // TOTP input
  var bothMode = authMode === "both";              // show both side by side

  var subtitle = "Enter your passphrase to continue";
  if (authMode === "totp") subtitle = "Enter your authenticator code";
  if (authMode === "both") subtitle = "Enter passphrase and authenticator code";
  if (authMode === "either") subtitle = "Choose your authentication method";

  // Toggle HTML (only for "either" mode)
  ...
  var totpHidden = authMode === "either" ? ' style="display:none"' : '';
  var totpFieldHtml = showTotpField ?
    ...
    '<input id="totp" type="tel" maxlength="6" inputmode="numeric" placeholder="6-digit code" autocomplete="one-time-code"' + (authMode === "totp" ? ' autofocus' : '') + '></div>' : '';
  ...
  return '<!DOCTYPE html><html lang="en"><head>' +
    ...
    'var currentMode=authMode==="either"?"passphrase":(authMode==="totp"?"totp":"passphrase");' +
    // Toggle handlers (only for "either" mode)
    'if(authMode==="either"){' +
