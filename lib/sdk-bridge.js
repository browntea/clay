...
function createSDKBridge(opts) {
  ...
  // Resolve the context window size for a model name. Used to set
  // autoCompactWindow so the SDK auto-compacts before context overflow.
  // Mirrors the client-side KNOWN_CONTEXT_WINDOWS in public/modules/app-panels.js
  // — keep both in sync if you add or change models.
  var MODEL_CONTEXT_WINDOWS = {
    ...
    "o3": 200000,
    "o4-mini": 200000,
    // MiniMax models via LiteLLM. LiteLLM does NOT pass a max_input_tokens limit,
    // and the upstream api.minimaxi.com/anthropic endpoint fails with HTTP 400
    // "context window exceeds limit (2013)" well before the documented 200K limit.
    // Use a conservative 100K ceiling so SDK auto-compaction kicks in well before
    // the upstream rejects the request.
    "minimax-m2.7": 100000,
    "minimax-m3": 100000,
    "minimax-m2.7-zgf": 100000,
    "minimax-m3-zgf": 100000,
  };
  function resolveModelContextWindow(modelId) {
    if (!modelId) return undefined;
    var lc = modelId.toLowerCase();
    // [1m] suffix denotes 1M-token models
    if (lc.indexOf("[1m]") !== -1) return 1000000;
    for (var key in MODEL_CONTEXT_WINDOWS) {
      if (lc.indexOf(key) !== -1) return MODEL_CONTEXT_WINDOWS[key];
    }
    ...
    if (toolName === "AskUserQuestion") {
      return new Promise(function(resolve) {
        session.pendingAskUser[opts.toolUseID] = {
          resolve: resolve,
          input: input,
          mode: "canUseTool",  // tag so the finally block in processQueryStream
                                // can keep this entry — the resolve callback
                                // is still pending and the user's answer needs
                                // to find it via toolId.
        };
        if (opts.signal) {
          opts.signal.addEventListener("abort", function() {
    ...
    try {
      ...
      for (var j = 0; j < candidates.length; j++) {
        ...
        try {
          var procCwd;
          if (isMac) {
            // macOS: use lsof to get cwd (no /proc filesystem)
            var lsofOut = execSync("lsof -a -p " + c.pid + " -d cwd -Fn 2>/dev/null", { encoding: "utf8", timeout: 3000 });
            var cwdMatch = lsofOut.match(/\nn(.+)/);
            procCwd = cwdMatch ? cwdMatch[1] : null;
          } else {
            // Linux: use /proc/<pid>/cwd symlink
            procCwd = fs.readlinkSync("/proc/" + c.pid + "/cwd");
          }
          if (procCwd === cwd) {
            results.push(c);
          }
        } catch (e) {
          // Process may have exited between ps and lsof/proc check — skip it
        }
      }
      return results;
    } catch (e) {
  ...
  async function processQueryStream(session) {
    ...
    try {
      ...
      // Preserve MCP- and canUseTool-mode AskUserQuestion entries across turn
      // boundaries. The MCP path is intentionally stateless: the tool returns
      // immediately ("card posted, end your turn") and the user's answer is
      // expected to arrive as a brand-new user_message on the *next* turn. The
      // canUseTool path (Claude's native path) blocks the SDK on a permission
      // callback; if we drop the pending entry here, the user's answer arrives
      // before the next query starts and the ask_user_response handler can't
      // find the toolId and silently drops it. Both modes MUST survive this
      // finally block; only entries whose resolve callback is no longer
      // useful (e.g. queue was abandoned) should be cleared.
      var keepAskUser = {};
      for (var _tid in session.pendingAskUser) {
        var _pending = session.pendingAskUser[_tid];
        if (_pending && (_pending.mode === "mcp" || _pending.mode === "canUseTool")) {
          keepAskUser[_tid] = _pending;
        }
      }
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    var queryOpts = {
      cwd: cwd,
      model: queryModel,
      // Tell the SDK the model's context window so it can auto-compact before
      // reaching 100%. Without this, MiniMax (and other 3P providers via
      // LiteLLM) fail with "context window exceeds limit (2013)" because
      // getContextUsage is unsupported and the SDK cannot measure usage.
      autoCompactWindow: resolveModelContextWindow(queryModel),
    ...
    try {
      ...
      console.error("[sdk-bridge] Failed to create query for session " + session.localId + ":", errMsg);
      ...
      // Auto-recovery: when the SDK session file is gone (e.g. ~/.claude/projects/<id>/ deleted),
      // resumeSessionId becomes invalid. Detect this specific error, drop the dead cliSessionId,
      // notify the user, and let the next user message start a fresh SDK session.
      var isResumeGone = session.cliSessionId
        && /no conversation found|conversation not found|resume.*not found/i.test(errMsg);
      if (isResumeGone && !session._resumeRecovered) {
        console.log("[sdk-bridge] SDK session " + session.cliSessionId + " is gone — dropping cliSessionId; next send will start a fresh session");
        session._resumeRecovered = true;
        session.cliSessionId = null;
        session.recoveredFromLostSdk = true;
        sm.saveSessionFile(session);
        sm.broadcastSessionList();
        sendAndRecord(session, {
          type: "info",
          text: "Previous SDK session was lost. Re-send your message to start a fresh session — earlier messages stay in this Clay session's history but the AI has no memory of them yet.",
        });
      }
      ...
      sendAndRecord(session, { type: "error", text: "Failed to start query: " + errMsg });
      sendAndRecord(session, { type: "done", code: 1 });
      sm.broadcastSessionList();
      return;
    }
