...
function createSDKBridge(opts) {
  ...
  async function startQuery(session, text, images, linuxUser) {
    ...
    // Store handle on session for iteration and control.
    // For in-process, queryInstance is the raw SDK query. For worker, it's the handle itself.
    session.queryInstance = handle._rawQuery || handle;
    session.messageQueue = handle._messageQueue || handle;
  ...
  function pushMessage(session, text, images) {
    ...
    session.lastActivityAt = Date.now();
    // Route through message queue (works for both in-process and worker handles)
    if (session.messageQueue && typeof session.messageQueue.push === "function") {
      session.messageQueue.push(userMsg);
    } else if (session.queryInstance && typeof session.queryInstance._pushRaw === "function") {
      // Worker handle: queryInstance is the handle itself
      session.queryInstance._pushRaw(userMsg);
    }
  }
