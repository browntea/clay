...
function createProjectContext(opts) {
  ...
  // Gate clay-browser on the Chrome extension being actively connected.
  // Without this, browser tools appear in every session's tool list and the
  // model may reach for them when no extension is present — which both wastes
  // context and can cause it to pick the wrong browser MCP when the user has
  // another one configured (see issue #325).
  function getLocalMcpServers() {
    if (!mcpServers) return undefined;
    var ws = browserState._extensionWs;
    var extConnected = !!(ws && ws.readyState === 1);
    if (extConnected) return mcpServers;
    var keys = Object.keys(mcpServers);
    var filtered = {};
    var hasAny = false;
    for (var i = 0; i < keys.length; i++) {
      if (keys[i] === "clay-browser") continue;
      filtered[keys[i]] = mcpServers[keys[i]];
      hasAny = true;
    }
