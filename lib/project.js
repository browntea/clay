...
function createProjectContext(opts) {
  ...
  function restoreDebateState(ws) {
    ...
    sm.sessions.forEach(function (session) {
      ...
      if (phase === "preparing") {
        ...
        console.log("[debate] Restoring debate (live). topic:", debate.topic);
  ...
  function handleModeratorTurnDone(session, fullText) {
    ...
    // Save to session history
    var turnEntry = { type: "debate_turn_done", mateId: debate.moderatorId, mateName: moderatorProfile.name, role: "moderator", text: fullText, avatarStyle: moderatorProfile.avatarStyle, avatarSeed: moderatorProfile.avatarSeed, avatarColor: moderatorProfile.avatarColor };
    ...
    // Check max rounds
    if (debate.round >= MAX_DEBATE_ROUNDS) {
      // Ask moderator to wrap up
      debate.turnInProgress = true;
      sendToSession(session.localId, {
        type: "debate_turn",
        mateId: debate.moderatorId,
      ...
      debate.moderatorSession.pushMessage(
        "[SYSTEM: Maximum rounds reached. Please provide your closing summary now. Do NOT mention any panelist.]",
        buildModeratorCallbacks(session)
      );
      return;
    }
  ...
  function handlePanelistTurnDone(session, mateId, fullText) {
    ...
    // Save to session history
    var turnEntry = { type: "debate_turn_done", mateId: mateId, mateName: profile.name, role: panelistInfo ? panelistInfo.role : "", text: fullText, avatarStyle: profile.avatarStyle, avatarSeed: profile.avatarSeed, avatarColor: profile.avatarColor };
    session.history.push(turnEntry);
    sm.appendToSessionFile(session, turnEntry);
    sendToSession(session.localId, turnEntry);
  ...
  function handleDebateComment(ws, msg) {
    ...
    if (debate.turnInProgress && debate._currentTurnMateId && debate._currentTurnMateId !== debate.moderatorId) {
      ...
      var turnEntry = { type: "debate_turn_done", mateId: abortMateId, mateName: profile.name, role: panelistInfo ? panelistInfo.role : "", text: partialText, avatarStyle: profile.avatarStyle, avatarSeed: profile.avatarSeed, avatarColor: profile.avatarColor, interrupted: true };
      session.history.push(turnEntry);
      sm.appendToSessionFile(session, turnEntry);
  ...
  function rebuildDebateState(session, ws) {
    ...
    for (var i = 0; i < session.history.length; i++) {
      ...
      if (h.type === "debate_turn_done") roundCount++;
    }
    ...
    var debate = {
      ...
      turnInProgress: false,
      pendingComment: null,
      round: Math.max(1, Math.ceil(roundCount / 2)),
      history: [],
      awaitingConcludeConfirm: false,
