...
import { updateSettingsModels, updateSettingsStats, updateDaemonConfig, handleSetPinResult, handleKeepAwakeChanged, handleAutoContinueChanged, handleRestartResult, handleShutdownResult, handleSharedEnv, handleSharedEnvSaved, handleGlobalClaudeMdRead, handleGlobalClaudeMdWrite, handleSetupTotpResult, handleDisableTotpResult, handleVerifyTotpResult, handleSetAuthModeResult, handleShowTotpQrResult } from './server-settings.js';
...
export function processMessage(msg) {
    ...
    switch (msg.type) {
      ...
      case "session_switched":
        ...
        if (msg.vendor) {
          // Always apply the session's vendor on switch — the vendor is
          // structurally bound to the session, not to the previous UI state.
          // Unlock first so a switch from Codex→Claude (or vice versa)
          // correctly updates currentVendor even when the previous session
          // had locked the selection.
          var _prevVendor = store.get('currentVendor');
          if (msg.vendor !== _prevVendor) {
      ...
      case "setup_totp_result":
        handleSetupTotpResult(msg);
        break;

      case "show_totp_qr_result":
        handleShowTotpQrResult(msg);
        break;

      case "disable_totp_result":
        handleDisableTotpResult(msg);
        break;

      case "verify_totp_result":
        handleVerifyTotpResult(msg);
        break;

      case "set_auth_mode_result":
        handleSetAuthModeResult(msg);
        break;
