...
import { showLoopBanner, updateLoopBanner, updateRalphBars, updateLoopInputVisibility, showRalphApprovalBar, updateRalphApprovalStatus, openRalphPreviewModal, showExecModal, updateExecModalStatus } from './app-loop-ui.js';
    ...
    switch (msg.type) {
      ...
      case "context_usage":
        if (msg.data && !store.get('replayingHistory')) {
          ...
          var _hce = store.get('headerContextEl');
          if (_hce) _hce.removeAttribute("data-tip");
          if (store.get('ctxPopoverVisible')) renderCtxPopover();
        }
        break;
      ...
      case "mate_deleted":
        ...
        if (_s3.dmMode && store.get('dmTargetUser') && store.get('dmTargetUser').id === msg.mateId) {
          exitDmMode();
        }
        break;
