...
function showIconCtxMenu(anchorEl, slug, name) {
  ...
  if (isWorktree) {
    ...
    removeWtItem.addEventListener("click", function (e) {
      e.stopPropagation();
      closeProjectCtxMenu();
      if (getWs() && store.getState().connected) {
...
function showProjectCtxMenu(anchorEl, slug, name, icon, position) {
  ...
  // --- Project Settings ---
  if (!store.getState().permissions || store.getState().permissions.projectSettings !== false) {
    ...
    settingsItem.addEventListener("click", function (e) {
      e.stopPropagation();
      closeProjectCtxMenu();
      openProjectSettings(slug, { slug: slug, name: name, icon: icon, projectOwnerId: store.getState().currentProjectOwnerId, ownerLocked: store.getState().ownerLocked });
    });
  ...
  // --- Manage Access ---
  if (store.getState().isMultiUserMode && slug.indexOf("--") === -1) {
    var isProjectOwner = store.getState().myUserId && store.getState().currentProjectOwnerId && store.getState().myUserId === store.getState().currentProjectOwnerId;
    var isAdmin = store.getState().permissions && store.getState().permissions.projectSettings !== false;
  ...
  if (!store.getState().permissions || store.getState().permissions.deleteProject !== false) {
...
function showProjectRename(slug, currentName) {
  ...
  function commitRename() {
    ...
    if (newName && newName !== currentName && getWs() && store.getState().connected) {
...
function showTrashZone() {
  ...
  trash.addEventListener("drop", function (e) {
    ...
    if (slug && getWs() && store.getState().connected) {
