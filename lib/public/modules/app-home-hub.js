...
function renderHomeHubMates() {
  ...
  if (!container) return;
  container.innerHTML = "";
  if (!store.getState().cachedMatesList || store.getState().cachedMatesList.length === 0) {
    container.classList.add("hidden");
    return;
  }
  container.classList.remove("hidden");
  for (var i = 0; i < store.getState().cachedMatesList.length; i++) {
    (function (mate) {
      var item = document.createElement("div");
    ...
    })(store.getState().cachedMatesList[i]);
  }
}

export function showHomeHub() {
  if (store.getState().dmMode) exitDmMode();
  homeHubVisible = true;
  homeHub.classList.remove("hidden");
  // Show close button only if there's a project to return to
  if (hubCloseBtn) {
    if (store.getState().currentSlug) hubCloseBtn.classList.remove("hidden");
