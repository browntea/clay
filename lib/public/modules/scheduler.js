...
function enforceMinTime() {
  ...
  if (isToday) {
    // Round up to the next quarter-hour for min
    var minMinutes = now.getHours() * 60 + now.getMinutes();
    var nextQuarter = Math.ceil(minMinutes / 15) * 15;
    var minH = Math.floor(nextQuarter / 60);
    var minM = nextQuarter % 60;
    if (minH >= 24) { minH = 23; minM = 45; }
