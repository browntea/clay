...
var sidebarTab = "tasks";          // "tasks" | "library"
var libraryEntries = [];           // cached loop library entries
...
function ensurePanel() {
  ...
  sidebarHeader.innerHTML =
    '<div class="scheduler-sidebar-tabs">' +
      '<button class="scheduler-sidebar-tab active" data-tab="tasks">' +
        '<span>Tasks</span><span class="scheduler-sidebar-count" id="scheduler-tasks-count">0</span>' +
      '</button>' +
      '<button class="scheduler-sidebar-tab" data-tab="library">' +
        '<span>Library</span><span class="scheduler-sidebar-count" id="scheduler-library-count">0</span>' +
      '</button>' +
    '</div>' +
  ...
  // Tab click handlers
  var tabBtns = sidebarHeader.querySelectorAll(".scheduler-sidebar-tab");
  for (var ti = 0; ti < tabBtns.length; ti++) {
    (function (tbtn) {
      tbtn.addEventListener("click", function (e) {
        e.stopPropagation();
        var tab = tbtn.dataset.tab;
        if (sidebarTab === tab) return;
        sidebarTab = tab;
        for (var tj = 0; tj < tabBtns.length; tj++) {
          tabBtns[tj].classList.toggle("active", tabBtns[tj] === tbtn);
        }
        if (sidebarTab === "library") {
          // Request library entries from server
          send({ type: "loop_library_scan" });
        }
  ...
  var sidebarLibrary = document.createElement("div");
  sidebarLibrary.className = "scheduler-sidebar-list scheduler-library-list hidden";
  sidebar.appendChild(sidebarLibrary);
  sidebarLibraryEl = sidebarLibrary;
...
function renderSidebar() {
  ...
  var countEl = panel ? panel.querySelector("#scheduler-tasks-count") : null;
...
function updateSidebarTabVisibility() {
  if (sidebarListEl) sidebarListEl.classList.toggle("hidden", sidebarTab !== "tasks");
  if (sidebarLibraryEl) sidebarLibraryEl.classList.toggle("hidden", sidebarTab !== "library");
  // Show/hide add-row and ralph toggle based on tab
  var addRow = panel ? panel.querySelector(".scheduler-add-row") : null;
  if (addRow) addRow.classList.toggle("hidden", sidebarTab !== "tasks");
  var ralphToggle = panel ? panel.querySelector("#scheduler-ralph-toggle") : null;
  if (ralphToggle) ralphToggle.classList.toggle("hidden", sidebarTab !== "tasks");
  if (sidebarTab === "library") {
    renderLibrary();
  }
}
...
function renderLibrary() {
  if (!sidebarLibraryEl) return;

  // Update library count badge
  var libCountEl = panel ? panel.querySelector("#scheduler-library-count") : null;
  if (libCountEl) libCountEl.textContent = libraryEntries.length;

  if (libraryEntries.length === 0) {
    sidebarLibraryEl.innerHTML =
      '<div class="scheduler-empty">' +
        '<div style="margin-bottom:8px">No library loops yet</div>' +
        '<div class="scheduler-library-hint">Create loops in<br><code>.claude/loop-library/{slug}/</code><br>with a PROMPT.md file</div>' +
      '</div>';
    return;
  }
  ...
  for (var i = 0; i < libraryEntries.length; i++) {
    ...
    html += '<div class="scheduler-library-card" data-slug="' + esc(entry.slug) + '">';
    html += '<div class="scheduler-library-card-header">';
    html += '<div class="scheduler-library-card-name">' + esc(entry.description || entry.slug) + '</div>';
    if (entry.hasJudge) {
      html += '<span class="scheduler-library-badge judge" title="Has JUDGE.md — iterative loop">' + iconHtml("scale") + '</span>';
    } else {
      html += '<span class="scheduler-library-badge single" title="No JUDGE — single iteration">' + iconHtml("zap") + '</span>';
    }
    html += '</div>';
    // Tags
    if (entry.tags && entry.tags.length > 0) {
      html += '<div class="scheduler-library-tags">';
      for (var t = 0; t < entry.tags.length; t++) {
        html += '<span class="scheduler-library-tag">' + esc(entry.tags[t]) + '</span>';
      }
      html += '</div>';
    }
    // Meta row
    html += '<div class="scheduler-library-meta">';
    html += '<span class="scheduler-library-slug">' + esc(entry.slug) + '</span>';
    if (entry.hasJudge) {
      html += '<span class="scheduler-library-iter">max ' + entry.maxIterations + ' iter</span>';
    }
    html += '</div>';
    // Actions
    html += '<div class="scheduler-library-actions">';
    html += '<button class="scheduler-library-btn preview" data-action="preview" data-slug="' + esc(entry.slug) + '" title="Preview">Preview</button>';
    html += '<button class="scheduler-library-btn run" data-action="run" data-slug="' + esc(entry.slug) + '" title="Run">Run</button>';
    html += '</div>';
    html += '</div>';
  }
  sidebarLibraryEl.innerHTML = html;

  // Attach click handlers
  var cards = sidebarLibraryEl.querySelectorAll(".scheduler-library-card");
    ...
    (function (card) {
      ...
      if (previewBtn) {
        previewBtn.addEventListener("click", function (e) {
          ...
          send({ type: "loop_library_read", slug: slug });
          // Show in detail pane
          switchMode("detail");
          if (contentDetailEl) {
            contentDetailEl.innerHTML =
              '<div class="scheduler-detail-header">' +
                '<button class="scheduler-detail-back">' + iconHtml("arrow-left") + ' Back</button>' +
                '<span class="scheduler-detail-title">' + esc(slug) + '</span>' +
              '</div>' +
              '<div class="scheduler-detail-loading">Loading…</div>';
            var backBtn = contentDetailEl.querySelector(".scheduler-detail-back");
            if (backBtn) {
              backBtn.addEventListener("click", function () {
                switchMode("calendar");
              });
            }
            try { lucide.createIcons({ node: contentDetailEl }); } catch (e2) {}
          }
        });
      }
      // Run button
      ...
      if (runBtn) {
        runBtn.addEventListener("click", function (e) {
          e.stopPropagation();
          var slug = card.dataset.slug;
          send({ type: "loop_library_run", slug: slug });
          showToast("Starting loop: " + slug, "success");
        });
      }
    })(cards[ci]);
  }

  try { lucide.createIcons({ node: sidebarLibraryEl }); } catch (e) {}
}

function renderLibraryPreview(msg) {
  if (!contentDetailEl || currentMode !== "detail") return;
  var html = '<div class="scheduler-detail-header">';
  html += '<button class="scheduler-detail-back">' + iconHtml("arrow-left") + ' Back</button>';
  html += '<span class="scheduler-detail-title">' + esc(msg.slug) + '</span>';
  html += '<button class="scheduler-library-btn run" id="library-preview-run" title="Run this loop">Run</button>';
  html += '</div>';
  // Tabs for prompt/judge
  ...
  if (msg.meta && Object.keys(msg.meta).length > 0) {
    html += '<button class="scheduler-detail-tab" data-tab="meta">LOOP.json</button>';
  }
  html += '</div>';
  html += '<div id="scheduler-detail-body" class="scheduler-detail-body">';
  html += msg.prompt ? '<div class="md-content">' + renderMarkdown(msg.prompt) + '</div>' : '<div class="scheduler-empty">Empty PROMPT.md</div>';
  html += '</div>';
  ...
  var backBtn = contentDetailEl.querySelector(".scheduler-detail-back");
  if (backBtn) {
    backBtn.addEventListener("click", function () { switchMode("calendar"); });
  }
  // Run button
  var runBtn = contentDetailEl.querySelector("#library-preview-run");
  if (runBtn) {
    runBtn.addEventListener("click", function () {
      send({ type: "loop_library_run", slug: msg.slug });
      showToast("Starting loop: " + msg.slug, "success");
    });
  }
  // Tab switching
  var tabs = contentDetailEl.querySelectorAll(".scheduler-detail-tab");
  var bodyEl2 = contentDetailEl.querySelector("#scheduler-detail-body");
  for (var i = 0; i < tabs.length; i++) {
    (function (tab) {
      tab.addEventListener("click", function () {
        ...
        if (which === "prompt") {
          bodyEl2.innerHTML = msg.prompt ? '<div class="md-content">' + renderMarkdown(msg.prompt) + '</div>' : '<div class="scheduler-empty">Empty</div>';
        } else if (which === "judge") {
          bodyEl2.innerHTML = msg.judge ? '<div class="md-content">' + renderMarkdown(msg.judge) + '</div>' : '<div class="scheduler-empty">No JUDGE.md</div>';
        } else if (which === "meta") {
          bodyEl2.innerHTML = '<pre class="scheduler-library-meta-json">' + esc(JSON.stringify(msg.meta, null, 2)) + '</pre>';
        }
      });
    })(tabs[i]);
  }
...
// --- Loop Library message handlers ---

export function handleLibraryEntries(msg) {
  libraryEntries = msg.entries || [];
  var libCountEl = panel ? panel.querySelector("#scheduler-library-count") : null;
  if (libCountEl) libCountEl.textContent = libraryEntries.length;
  if (panelOpen && sidebarTab === "library") {
    renderLibrary();
  }
}

export function handleLibraryEntryContent(msg) {
  // Show preview in detail pane
  renderLibraryPreview(msg);
}

export function handleLibraryLoaded(msg) {
  showToast("Loop loaded from library: " + (msg.slug || ""), "success");
  // Refresh registry
  send({ type: "loop_registry_list" });
}

export function handleLibraryError(msg) {
  showToast(msg.text || "Library error", "error");
}
