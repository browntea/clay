    ...
    steps: [
      ...
      },
      {
        ...
        body: function () {
          ...
          var html = "";
          if (isInsecure) {
            html += "⚠️ <strong>Insecure context detected.</strong><br><br>" +
              "Notifications require a <strong>secure context</strong> (HTTPS or localhost). " +
              "You're accessing Clay over <strong>http://" + location.hostname + "</strong>, so the browser silently blocks notifications.<br><br>" +
            ...
            html += "The browser says permission is granted, but the notification didn't show.<br><br>";

            // Untrusted HTTPS cert case
            if (hasUntrustedCert) {
