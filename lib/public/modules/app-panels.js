...
var _contextHighWarnShown = false;  // Track if we've warned about high context usage
...
var KNOWN_CONTEXT_WINDOWS = {
  ...
  // See sdk-bridge.js MODEL_CONTEXT_WINDOWS — LiteLLM doesn't set max_input_tokens
  // and upstream MiniMax rejects well before the documented 200K ceiling.
  "minimax-m2.7": 100000,
  "minimax-m3": 100000,
...
function rebuildModelList() {
  ...
  // Picker visibility: always shown for all vendors and modes.
  // Claude Agent SDK supports mid-session model switching via
  // queryInstance.setModel() (equivalent to CLI /model command).
  // Codex GUI locks after first message (protocol binds model at
  // thread creation).
...
export function updateContextPanel() {
  ...
  // Warn user when context runs high so they can /compact BEFORE the SDK tries to send and fails.
  // Two thresholds:
  //   60% — heads-up, gives plenty of buffer (most models)
  //   80% — urgent, compaction should run immediately
  // This is critical for MiniMax models where getContextUsage is unsupported and /compact itself
  // fails with "context window exceeds limit (2013)" once you cross ~95%.
  var lc = ((contextData && contextData.model) || "").toLowerCase();
  var isMinimax = lc.indexOf("minimax") !== -1;
  var warnThreshold = isMinimax ? 60 : 75;
  var urgentThreshold = isMinimax ? 80 : 88;
  if (win > 0 && pct >= warnThreshold && !_contextHighWarnShown && !store.get('replayingHistory')) {
    _contextHighWarnShown = true;
    if (pct >= urgentThreshold) {
      showToast(
        "Context at " + pct.toFixed(0) + "% — RUN /compact NOW (compactions fail above ~95%)",
        "warn"
      );
    } else {
      showToast(
        "Context at " + pct.toFixed(0) + "% — consider running /compact to free space",
        "warn"
      );
    }
  }
  // Reset once context drops well below threshold
  if (win > 0 && pct < (warnThreshold - 15)) {
    _contextHighWarnShown = false;
  }
}
