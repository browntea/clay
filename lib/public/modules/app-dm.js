...
export function enterDmMode(key, targetUser, messages) {
  ...
  store.setState({ dmMode: true, dmKey: key, dmTargetUser: targetUser });
  ...
  if (targetUser && targetUser.isMate) {
    ...
    if (!store.getState().mateProjectSlug && ws && ws.readyState === 1) {
  ...
  if (targetUser) {
    store.getState().dmUnread[targetUser.id] = 0;
  ...
  if (isMate && targetUser.projectSlug) {
    ...
    var myUser = store.getState().cachedAllUsers.find(function (u) { return u.id === store.getState().myUserId; });
    if (!myUser) {
      try { var cached = JSON.parse(localStorage.getItem("clay_my_user") || "null"); if (cached) myUser = cached; } catch(e) {}
    }
    var myAvatarUrl = userAvatarUrl(myUser || { id: store.getState().myUserId }, 36);
  ...
  // Render DM messages
  store.setState({ dmMessageCache: messages ? messages.slice() : [] });
...
export function exitDmMode(skipProjectSwitch) {
  if (!store.getState().dmMode) return;
  var wasMate = store.getState().dmTargetUser && store.getState().dmTargetUser.isMate;
  store.setState({ dmMode: false, dmKey: null, dmTargetUser: null });
  ...
  if (wasMate && !skipProjectSwitch) {
    ...
    store.setState({ returningFromMateDm: true, mateProjectSlug: null, savedMainSlug: null });
    ...
    if (ws && ws.readyState === 1) {
      ws.send(JSON.stringify({ type: "switch_session", id: store.getState().activeSessionId }));
...
export function handleMateCreatedInApp(mate, msg) {
  ...
  if (msg && msg.availableBuiltins) store.setState({ cachedAvailableBuiltins: msg.availableBuiltins });
  if (msg && msg.dmFavorites) store.setState({ cachedDmFavorites: msg.dmFavorites });
  ...
  if (!mate.builtinKey) {
    store.setState({ pendingMateInterview: mate });
  }
  openDm(mate.id);
}
...
export function updateMateIconStatus(msg) {
  if (!store.getState().mateProjectSlug) return;
  var slug = store.getState().mateProjectSlug;
...
export function connectMateProject(slug) {
  var s = store.getState();
  store.setState({ mateProjectSlug: slug });
  // Only save the main slug on the FIRST mate switch (preserve original main project)
  if (!s.savedMainSlug) store.setState({ savedMainSlug: s.currentSlug });
  store.setState({ currentSlug: slug, wsPath: "/p/" + slug + "/ws" });
  resetClientState();
  connect();
}

export function disconnectMateProject() {
  store.setState({ mateProjectSlug: null });
  ...
  var savedMainSlug = store.getState().savedMainSlug;
  if (savedMainSlug) {
    store.setState({
...
export function showDmTypingIndicator(typing) {
  ...
  var dmTargetUser = store.getState().dmTargetUser;
