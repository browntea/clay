...
export function ensureAssistantBlock() {
  ...
  if (!_el) {
    ...
    var dmTarget = store.getState().dmTargetUser;
    nameSpan.textContent = _isDm2 ? ((dmTarget && dmTarget.displayName) || "Mate") : "Claude Code";
...
export function addUserMessage(text, images, pastes, fromUserId, fromUserName) {
  ...
  var myUserId = store.getState().myUserId;
  ...
  var cachedAllUsers = store.getState().cachedAllUsers;
  var _targetUser;
  var _displayName;
  if (isOtherUser) {
...
export function addAuthRequiredMessage(msg) {
  ...
  if (msg.canAutoLogin) {
    ...
    loginBtn.addEventListener("click", function () {
      store.setState({ pendingTermCommand: "claude\n" });
...
export function showSuggestionChips(suggestion) {
  if (!suggestion || store.getState().processing) return;
