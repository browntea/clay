...
export function renderUserStrip(allUsers, onlineUserIds, myUserId, dmFavorites, dmConversations, dmUnread, dmRemovedUsers, matesList) {
  ...
  for (var mi = 0; mi < sortedMates.length; mi++) {
    (function (mate) {
      ...
      if (vendorIcons[mateVendor]) {
        var vendorBadge = document.createElement("img");
        vendorBadge.className = "icon-strip-vendor-badge";
        vendorBadge.src = vendorIcons[mateVendor];
        vendorBadge.alt = mateVendor;
        el.appendChild(vendorBadge);
      }
