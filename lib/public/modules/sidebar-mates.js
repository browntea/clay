...
function toggleDmUserPicker(anchorEl) {
  ...
  // Multi-user + disabled: inline hint so the user knows where to turn
  // Mates back on instead of just seeing a greyed empty section. The
  // hint deliberately omits `dm-picker-mates-only` so the .is-multi-user
  // CSS rule doesn't grey it out — the hint should stay legible.
  if (!matesEnabled && isMultiUser) {
    var disabledHint = document.createElement("div");
    disabledHint.className = "dm-user-picker-mates-disabled-hint";
    disabledHint.textContent = "Mates are off. Re-enable in User Settings \u2192 Mates.";
