...
function toggleDmUserPicker(anchorEl) {
  ...
  // --- MATES section ---
  // Marker class `dm-picker-mates-only` is the CSS gate. In single-user
  // it collapses the section entirely; in multi-user it greys it out
  // (still visible because the picker is shared with other users) and
  // we inject a hint pointing at the toggle in User Settings.
  ...
  // Disabled state: render a short hint above the (greyed-out) preview
  // list so users know what's going on and a clear button below the list
  // that deep-links into User Settings → Mates. The hint and the button
  // both omit `dm-picker-mates-only` so the CSS rule that greys out the
  // section doesn't grey them out — they need to stay legible and
  // clickable.
  if (!matesEnabled) {
    var disabledHint = document.createElement("div");
    disabledHint.className = "dm-user-picker-mates-disabled-hint";
    disabledHint.textContent = "Mates are off.";
    picker.appendChild(disabledHint);
  }
  ...
  function renderMatesList(filter) {
    ...
    // When Mates is disabled, render a small preview (first few) instead
    // of the full list. The preview is greyed/non-clickable via CSS, so
    // the user sees a hint of what's there without us pretending the
    // feature is available.
    if (!matesEnabled) {
      allMates = allMates.slice(0, 4);
    }
    if (filter) {
      var lf = filter.toLowerCase();
  ...
  if (matesEnabled) {
    // Create Mate option (normal state)
    ...
    // Disabled state: replace the Create entry with a button that
    // deep-links into User Settings → Mates so the user can flip the
    // toggle back on without having to remember where it lives.
    var openSettingsEl = document.createElement("button");
    openSettingsEl.type = "button";
    openSettingsEl.className = "dm-user-picker-open-settings";
    openSettingsEl.innerHTML = iconHtml("settings") + " <span>Open User Settings</span>";
    openSettingsEl.addEventListener("click", function () {
      closeDmUserPicker();
      openUserSettings('us-mates');
    });
    picker.appendChild(openSettingsEl);
  }
