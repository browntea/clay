...
import { getChatLayout, setChatLayout, openSettingsThemePicker } from './theme.js';
...
export function initUserSettings(appCtx) {
  ...
  // Theme picker. Replaces the older 2-button Light/Dark switcher with
  // the full picker built by theme.js (grouped: Dark / Light / Custom).
  // openSettingsThemePicker empties the container and mounts the picker
  // so subsequent theme registry updates re-render automatically.
  var themeSwitcher = document.getElementById('us-theme-switcher');
  if (themeSwitcher) {
    try { openSettingsThemePicker(themeSwitcher); } catch (e) {}
  }
