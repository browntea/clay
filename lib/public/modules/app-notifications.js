...
export function initAppNotifications(ctx) {
  _ctx = ctx;
...
export function handleNotificationCreated(msg) {
  ...
  var activeSession = _ctx.getActiveSessionId ? _ctx.getActiveSessionId() : null;
...
function navigateToNotification(notif) {
  if (notif.mateId && _ctx.openDm) {
    _ctx.openDm(notif.mateId);
    return;
  }

  var currentSlug = _ctx.getCurrentSlug ? _ctx.getCurrentSlug() : "";
  var needsProjectSwitch = notif.slug && notif.slug !== currentSlug;

  if (needsProjectSwitch && _ctx.switchProject) {
    // Store target session for after project switch
    if (notif.sessionId) {
      try { sessionStorage.setItem("pending-notif-session", notif.sessionId); } catch (e) {}
    }
    _ctx.switchProject(notif.slug);
...
function getProjectIcon(slug) {
  if (!_ctx.getCachedProjects) return null;
  var projects = _ctx.getCachedProjects();
...
function getProjectName(slug) {
  if (!_ctx.getCachedProjects) return slug;
  var projects = _ctx.getCachedProjects();
  for (var i = 0; i < projects.length; i++) {
