...
export function initHeader() {
  ...
  if (headerRenameBtn) {
    headerRenameBtn.addEventListener("click", function () {
      if (!store.getState().activeSessionId) return;
      ...
      function commit() {
        ...
        if (newTitle && newTitle !== currentText && ws && ws.readyState === 1) {
          ws.send(JSON.stringify({ type: "rename_session", id: store.getState().activeSessionId, title: newTitle }));
...
export function updateHistorySentinel() {
  ...
  if (store.getState().historyFrom > 0) {
    if (!existing) {
      ...
      historySentinelObserver = new IntersectionObserver(function (entries) {
        if (entries[0].isIntersecting && !store.getState().loadingMore && store.getState().historyFrom > 0) {
          requestMoreHistory();
        }
...
export function prependOlderHistory(items, meta) {
  ...
  var savedMsgEl = store.getState().currentMsgEl;
  var savedActivity = getActivityEl();
  var savedFullText = store.getState().currentFullText;
  ...
  // Restore saved state
  store.setState({ currentMsgEl: savedMsgEl, currentFullText: savedFullText });
  ...
  // Update state
  store.setState({ historyFrom: meta.from, loadingMore: false });
