...
export function openCreateModal(date, hour, anchorEl) {
  ...
  if (taskListEl) {
    ...
    html += '<div class="sched-create-task-item sched-create-task-add"><i data-lucide="plus"></i> Add new task</div>';
    ...
    for (var j = 0; j < items.length; j++) {
      (function (item) {
        item.addEventListener("click", function (e) {
          e.stopPropagation();
          if (item.classList.contains("sched-create-task-add")) {
            taskListEl.classList.add("hidden");
            closeCreateModal();
            configCtx.ctx.openRalphWizard("task");
            return;
          }
