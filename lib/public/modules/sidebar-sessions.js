...
function showSessionCtxMenu(anchorBtn, sessionId, title, cliSid, sessionData) {
  ...
  if (store.getState().isMultiUserMode && sessionData && sessionData.ownerId && sessionData.ownerId === store.getState().myUserId) {
  ...
  if (!store.getState().permissions || store.getState().permissions.sessionDelete !== false) {
    ...
    deleteItem.addEventListener("click", function (e) {
      ...
      showConfirm('Delete "' + (title || "New Session") + '"? This session and its history will be permanently removed.', function () {
        var ws = getWs();
        if (ws && store.getState().connected) {
...
function startInlineRename(sessionId, currentTitle) {
  ...
  function commitRename() {
    ...
    if (newTitle && newTitle !== currentTitle && getWs() && store.getState().connected) {
...
function renderSessionItem(s) {
  ...
  if (store.getState().isMultiUserMode && s.sessionVisibility === "private") {
