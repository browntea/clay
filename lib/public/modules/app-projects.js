...
export function updateProjectList(msg) {
  ...
  if (msg.serverUsers) {
    ...
    var prevOnlineIds = store.getState().cachedOnlineIds || [];
    store.setState({ cachedOnlineIds: newOnlineIds });
    renderTopbarPresence(msg.serverUsers);
    // Only re-render user strip if online IDs actually changed
    var st = store.getState();
    if (!msg.allUsers && st.cachedAllUsers.length > 0) {
      var onlineChanged = newOnlineIds.length !== prevOnlineIds.length || newOnlineIds.some(function (id, i) { return id !== prevOnlineIds[i]; });
      if (onlineChanged) {
        renderUserStrip(st.cachedAllUsers, newOnlineIds, st.myUserId, st.cachedDmFavorites, st.cachedDmConversations, st.dmUnread, st.dmRemovedUsers, st.cachedMatesList);
      }
    }
  }
  ...
  if (msg.allUsers) {
    store.setState({ cachedAllUsers: msg.allUsers });
    if (msg.dmFavorites) store.setState({ cachedDmFavorites: msg.dmFavorites });
    if (msg.dmConversations) store.setState({ cachedDmConversations: msg.dmConversations });
    var st2 = store.getState();
    renderUserStrip(msg.allUsers, st2.cachedOnlineIds, st2.myUserId, st2.cachedDmFavorites, st2.cachedDmConversations, st2.dmUnread, st2.dmRemovedUsers, st2.cachedMatesList);
...
export function resetClientState() {
  ...
  store.setState({ currentMsgEl: null });
  store.setState({ currentFullText: "" });
  resetToolState();
  clearPendingImages();
  setActivityEl(null);
  store.setState({ processing: false });
  setTurnCounter(0);
  store.setState({ messageUuidMap: [] });
  store.setState({ historyFrom: 0 });
  store.setState({ historyTotal: 0 });
  ...
  resetContext();
  resetRateLimitState();
  var headerCtx = store.getState().headerContextEl;
  if (headerCtx) { headerCtx.remove(); store.setState({ headerContextEl: null }); }
  hideSuggestionChips();
...
export function switchProject(slug) {
  ...
  if (isHomeHubVisible()) {
    hideHomeHub();
    if (slug === store.getState().currentSlug) return;
  }
  if (slug === store.getState().currentSlug) {
    var ws = getWs();
  ...
  resetScheduler(slug);
  store.setState({ currentSlug: slug });
  store.setState({ basePath: "/p/" + slug + "/" });
  store.setState({ wsPath: "/p/" + slug + "/ws" });
...
export function showUpdateAvailable(msg) {
  ...
  if (updatePillWrap && updateVersion && msg.version) {
    ...
    if (store.getState().isHeadlessMode) {
...
export function handleRemoveProjectResult(msg) {
  if (msg.ok) {
    ...
    if (msg.slug === currentSlug) {
      ...
      store.setState({ currentSlug: null });
      renderProjectList();
      resetClientState();
