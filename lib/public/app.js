...
// --- DOM refs ---
  ...
  function handleRemoveProjectCheckResult(msg) {
    ...
    if (msg.count > 0) {
      ...
      // No tasks — remove immediately (can be re-added easily)
      if (ws && ws.readyState === 1) {
  ...
  function renderRemovedProjectsList() {
    ...
    label.className = "add-project-removed-label";
    label.textContent = "Recently removed";
    addProjectRemoved.appendChild(label);
    for (var ri = 0; ri < cachedRemovedProjects.length; ri++) {
      ...
      var btn = document.createElement("button");
      btn.className = "add-project-removed-readd";
      btn.textContent = "Re-add";
      btn.dataset.path = rp.path;
      btn.addEventListener("click", function () {
