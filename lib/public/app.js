...
// --- DOM refs ---
  ...
  var wizardStep = 1;
  var wizardData = { task: "", maxIterations: 25 };
  ...
  function toggleLoopPopover() {
    var existing = document.getElementById("loop-popover");
    if (existing) {
      existing.remove();
      return;
    }
    var anchor = document.getElementById("loop-start-btn");
    if (!anchor) return;

    var pop = document.createElement("div");
    pop.id = "loop-popover";
    pop.className = "loop-popover";

    var statusText = "#" + loopIteration + "/" + loopMaxIterations;
    pop.innerHTML =
      '<div class="loop-popover-header">' +
        '<span class="loop-popover-icon">' + iconHtml("repeat") + '</span>' +
        '<span class="loop-popover-title">Ralph Loop</span>' +
      '</div>' +
      '<div class="loop-popover-row">' +
        '<span class="loop-popover-label">Progress</span>' +
        '<span class="loop-popover-value">' + statusText + '</span>' +
      '</div>' +
      '<div class="loop-popover-row">' +
        '<span class="loop-popover-label">Task</span>' +
        '<span class="loop-popover-value loop-popover-task">' + escapeHtml(wizardData.task || "—") + '</span>' +
      '</div>' +
      '<button class="loop-popover-stop">' + iconHtml("square") + ' Stop loop</button>';

    anchor.style.position = "relative";
    anchor.appendChild(pop);
    refreshIcons();

    pop.querySelector(".loop-popover-stop").addEventListener("click", function(e) {
      e.stopPropagation();
  ...
  function openRalphWizard() {
    wizardStep = 1;
    wizardData = { task: "", maxIterations: 25 };
  ...
  function wizardNext() {
    collectWizardData();
    if (wizardStep === 2 && !wizardData.task) {
      var taskEl = document.getElementById("ralph-task");
