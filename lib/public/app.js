...
// --- DOM refs ---
  ...
  function processMessage(msg) {
      switch (msg.type) {
        ...
        case "loop_started":
          ...
          var _rs = document.getElementById("ralph-sticky");
          if (_rs) { _rs.classList.add("hidden"); _rs.innerHTML = ""; }
          showLoopBanner(true);
          updateLoopButton();
  ...
  function updateLoopButton() {
    ...
    if (shouldShow) {
      if (!existing) {
        ...
        btn.innerHTML = '<i data-lucide="repeat"></i> <span>Ralph Loop</span>';
  ...
  function updateWizardStep() {
    ...
    if (skipBtn) skipBtn.style.display = (wizardStep <= 2 || wizardStep === 5) ? "none" : "";
    if (nextBtn) nextBtn.textContent = wizardStep === 5 ? "Launch" : wizardStep === 1 ? "Get Started" : "Next";
